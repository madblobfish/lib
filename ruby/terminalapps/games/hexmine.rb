...
class Minesweep < TerminalGame
  ...
  def draw
    move_cursor
    print @size.times.flat_map{|y|[y.odd? ? ' ' : '', *@size.times.map{|x|{'*'=>'#', '*!'=>'X'}.fetch(@map[[y,x]],@map[[y,x]])}].join(' ')}.join("\r\n")
  ...
  def uncover(x,y)
    ...
    around = ((a=[-1,0,1]).product(a)-[[0,0],*odd]).map{|a,b|[x+a, y+b]}.reject{|pos|pos.any?{|c|c<0 || c >= @size}}
